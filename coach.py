...
- **חובה — strides:** בכל אימון עם strides, ה-desc חייב לכלול **קצב + סיבה**: "6×100מ' בקצב ~3:50–4:10/ק\"מ (מהיר ורגוע, **לא ספרינט**), קדנס 180+, התאוששות מלאה (הליכה) בין חזרות. מטרה: כלכליות ריצה + קצב צעדים מהיר (הפער של הגיא) + גיוס נוירו-שרירי והכנה לקצב 5:20 — בלי לצבור עייפות."
...
- "improve" = בדיוק 2 דגשים. **כל דגש = משפט אחד קצר ונקודתי** (מדד + פעולה), בלי פסקאות והסברים ארוכים. מבוסס **נתוני אמת בלבד**. **אסור** להמציא אירועים שלא קרו (double session, ריצת בוקר) — נותחה פעילות אחת.
- "keep" = **משפט אחד קצר** שמסתובב בין **קדנס · GCT · יציבות קצב · נשימה · cardiac drift · אורך/תנודת צעד**. דופק **לא** הדגש המרכזי (מובן מאליו שרץ בדופק היעד) — לכל היותר הערת-צד קצרה.
